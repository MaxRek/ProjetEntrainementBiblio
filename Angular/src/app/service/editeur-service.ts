...
export class EditeurService {
  private apiUrl = 'http://localhost:8080/api/editeur';
  ...
  public findById(id: number): Observable<EditeurDto> {
   ...
   public save(editeurDto: EditeurDto): void {
    ...
    if (!editeurDto.id) {
      this.http.post<EditeurDto>(this.apiUrl, payload).subscribe(() => {
        this.refresh();
      });
    } else {
      this.http.put<EditeurDto>(`${ this.apiUrl }/${ editeurDto.id }`, payload).subscribe(() => {
        this.refresh();
      });
    }
  }
